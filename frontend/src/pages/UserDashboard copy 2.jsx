...
import { FaSadTear, FaFrown, FaMeh, FaSmile, FaGrinStars, FaShip, FaFish } from "react-icons/fa";
...
export default function UserDashboard() {
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-gray-900 overflow-hidden flex flex-col items-center justify-between">
      {/* Header */}
<header className="w-full bg-white dark:bg-gray-900 shadow-md border-b border-gray-300 dark:border-gray-700 flex items-center justify-center gap-3 py-4 fixed top-0 left-0 z-30">
      ...
      {dots.map((dot, i) => (
        <div
          key={i}
          className="absolute rounded-full bg-green-400 opacity-70 animate-blink"
          style={{
            top: dot.top,
            left: dot.left,
            width: dot.size,
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 z-10">
        {ratings.map((r) => (
          <RatingCard
            key={r.id}
            label={r.label}
