...
import { FaSearch, FaFileCsv, FaSadTear, FaFrown, FaMeh, FaSmile, FaGrinStars } from "react-icons/fa";
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (filterTo) temp = temp.filter((d) => new Date(d.created_at) <= new Date(filterTo));
      ...
      temp = temp.filter(
        (d) =>
          (d.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            d.feedback?.toLowerCase().includes(searchTerm.toLowerCase()))
      );
    }

    setFiltered(temp);
  }, [filterRating, filterFrom, filterTo, searchTerm, data]);
      ...
      {filtered.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-lg shadow-lg">
          <table className="w-full border-collapse">
            ...
            <tbody>
              {filtered.map((d, idx) => {
                const bgColor = ratingMap[d.rating]?.color + "33";
                return (
                  <tr
                    key={d.id}
                    className={`text-center ${idx % 2 === 0 ? "bg-white" : "bg-gray-50"} hover:bg-green-50`}
                    style={{ backgroundColor: bgColor }}
                  >
                    <td className="px-4 py-2 border-b">{d.id}</td>
                    <td className="px-4 py-2 border-b font-bold" style={{ color: ratingMap[d.rating]?.color }}>
                      {d.rating}
                    </td>
